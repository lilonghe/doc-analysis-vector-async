...
export const api = {
  ...
  getAllFilesStatus: async (): Promise<StatusResponse> => {
    const response = await axios.get(`${API_BASE_URL}/api/files/status`);
    return response.data;
  },
  ...
  processAllFiles: async () => {
    const response = await axios.post(`${API_BASE_URL}/api/process-all`);
    return response.data;
  },
  ...
  getFileChunks: async (fileId: string) => {
    const response = await axios.get(`${API_BASE_URL}/api/files/${fileId}/chunks`);
    return response.data;
  },
};